...
public partial class MovementToySelfTest : Node
{
        ...
        // Last, because it moves the world seed; every entry must generate a valid stage with what it promises.
        {
            ...
            foreach (var e in SampleStages.All)
            {
                ...
                bool shows = e.Name switch
                {
                    ...
                    "dive" => d.OptionalLines.Any(l => l.IsTunnel && l.CoverStart >= 0 && l.RidgeHeight < 0f),
                ...
                if (!ok)
                {
                    ...
                    for (int s2 = 1; s2 <= 200 && found.Count < 5; s2++)
                    {
                        ...
                        bool shows2 = e.Name switch
                        {
                            ...
                            "dive" => d2.OptionalLines.Any(l => l.IsTunnel && l.CoverStart >= 0 && l.RidgeHeight < 0f),
            ...
            int canyonSample = System.Array.FindIndex(SampleStages.All, s => s.Name == "tunnels + pit") + 1;
            _debug.Tuning.World.SampleStage = canyonSample;
            foreach (var _ in Frames(4)) yield return null;
            var w = _debug.World;
            Check("World › Sample Stage rebuilds the named stage from the panel", w.IsStage && w.Seed == SampleStages.All[canyonSample - 1].Seed && w.Archetype == TerrainArchetype.CanyonRun, $"stage={w.IsStage} seed={w.Seed} {w.Archetype}");
    ...
    private void RunStageGenerationBatchCase(TerrainArchetype archetype)
    {
        ...
        int tunnels = 0, tunnelsCovered = 0, seedsWithTunnel = 0, tunnelsDroppedFlight = 0, tunnelsDroppedCover = 0; float coveredSum = 0f, coveredMin = float.MaxValue, coveredMax = 0f; List<int> tunnelSeeds = new();
        ...
        for (int i = 0; i < Count; i++)
        {
            ...
            if (def.DroppedDetail.Contains("tunnel line")) { tunnelsDroppedFlight++; if (System.Environment.GetEnvironmentVariable("RUSHCORE_LINE_TRACE") == "1") GD.Print($"[SELFTEST] {A} seed {req.RunSeed}/{req.StageIndex} dropped a dive: {def.DroppedDetail}"); }
            if (def.DroppedDetail.Contains("no covered run")) tunnelsDroppedCover++;
            ...
            if (measure) MeasureSeed(gen, req, def, wall, floor3Room, archetype);
        }
        sw.Stop();
        if (measure) PrintMeasureTables(A, archetype, wall, floor3Room, seedsWithFloor3, Count);
        ...
        if (tunnels > 0 || tunnelsDroppedFlight + tunnelsDroppedCover > 0) GD.Print($"[SELFTEST] {A} tunnels over the batch: {tunnels} tunnel lines, {tunnelsCovered} covered on {seedsWithTunnel} seeds; covered {coveredMin:0}–{coveredMax:0} m (avg {(tunnelsCovered > 0 ? coveredSum / tunnelsCovered : 0f):0} m); stages that dropped one: {tunnelsDroppedFlight} for a flight at the cap, {tunnelsDroppedCover} uncovered; at stage 0: {string.Join(" ", tunnelSeeds)}");
        ...
        // Dives (docs/13 §2.2, D-116): the open landscapes host tunnels beneath the ground; every placed one passes its validators
        // (the batch's first check), and each archetype keeps its measured share of seeds with one (the batch floors, as D-100's).
        if (archetype != TerrainArchetype.CanyonRun)
        {
    ...
    private IEnumerable RunGeneratedStageCase()
    {
        ...
        // drive and the branch ride all stay on the stage this case set up.
        ...
        // Cosmetic scatter (04 §5G, 06 §16; T4): nothing stands in a line, a pad, an anchor, a lid or the
        // spiral disc; the edge markers deliberately do stand at the level width, and carry no collider.
        {
            ...
            Check("no cosmetic scatter stands inside a line, exit pad, anchor, lid or the spiral disc (04 §5G)",
        ...
        // The tunnel drive (docs/13 §2.8, D-113, D-116), on every archetype: from the primary before a tunnel's fork the
        // follower takes the line into the rock (a canyon's portal, or an open landscape's dive down its open cut) and out
        // the far portal to the rejoin. Inside the covered run the ball stays grounded, no portal is an impact, the lens
        // stays under the arch and inside the walls, and the exit speed matches the model. Then the cap: a ball set down on
        // the ground over the covered run rests on it as a floor, and driven along it stays on it. A drive seed without a
        // covered tunnel borrows the first seed of its archetype with one.
        int tunnelSeed = 0;
        {
            if (!stage.OptionalLines.Any(l => l.IsTunnel && l.CoverStart >= 0))
            {
                ...
                for (int sd = 1; sd <= 60; sd++)
                {
                    var d3 = gen3.Generate(new StageGenerationRequest(sd, 0, world.Archetype));
            ...
            Check("a stage with a covered tunnel builds its roofs (docs/13 §2.1)", tunnel is null || world.TunnelRoofTriangles > 0, $"roof triangles={world.TunnelRoofTriangles}");
            if (tunnel is not null)
            {
                ...
                if (System.Environment.GetEnvironmentVariable("RUSHCORE_TUNNEL_TRACE") == "1")
                    for (int i = Mathf.Max(0, tunnel.CoverStart - 100); i <= Mathf.Min(tv.Count - 1, tunnel.CoverStart + 10); i += 2)
                    {
                        var (fl, wgt, wo) = hf.TunnelFloorAt(tunnel, tv[i].Position.X, tv[i].Position.Z);
                        GD.Print($"[TUNNEL-FLOOR] i {i} d {tv[i].Distance:0} y {tv[i].Position.Y:0.00} floor {fl:0.00} w {wgt:0.00} without {wo:0.00} grid {world.GridHeight(tv[i].Position.X, tv[i].Position.Z):0.00}");
                    }
                ...
                while (pt++ < Engine.PhysicsTicksPerSecond * 40)
                {
                    ...
                    if (tunnel.RidgeHeight < 0f && ti == tunnel.CoverStart - 70) Shot("dive_open_cut_ahead");
                    ...
                    if (trace && !inside && ti >= tunnel.CoverStart - 150 && ti < tv.Count)
                        GD.Print($"[TUNNEL-PRE] t{pt} ti {ti} pos ({p.X:0.0},{p.Y:0.00},{p.Z:0.0}) floor {tv[ti].Position.Y:0.00} ground {world.SampleHeight(p.X, p.Z, p.Y):0.00} v ({_player.Velocity.X:0.0},{_player.Velocity.Y:0.0},{_player.Velocity.Z:0.0}) |v| {_player.Velocity.Length():0.0} g {_player.IsGrounded} raw {_player.IsRawGrounded} follow {_player.GroundFollowActive} n.y {_player.GroundNormal.Y:0.00} c {_player.GetContactCount()}");
                    if (inside)
                    {
                        insideTicks++;
                ...
                GD.Print($"[SELFTEST] {(tunnel.RidgeHeight < 0f ? "dive" : "tunnel")} drive (seed {world.Seed}): fork at {verts[tunnel.JoinStart].Distance:0} m, covered {coverStart:0}→{coverEnd:0} m of {tunnel.Length:0}; {insideTicks} ticks inside, grounded {insideFrac:P0}, min speed inside {(insideTicks > 0 ? minSpeedInside : 0f):0.0} m/s, portal impacts {portalImpacts}, exit {exitSpeed:0.0} m/s vs model {modelExit:0.0}; lens: {lensInside} ticks under the arch ({lensAbove} above it by up to {lensAboveBy:0.00} m), {lensOutside} in the rock; reached the rejoin {reachedEnd}, max {maxOff:0} m off the path");
                ...
                // The cap drive (docs/13 §2.8, D-116): driven along the surface over the covered run, from a fifth in to a fifth
                // before its end, the ball stays on the cap (grounded, never below the cap's top: the trench under it is never met).
                {
                    int fifth = Mathf.Max(2, (tunnel.CoverEnd - tunnel.CoverStart) / 5);
                    int from = tunnel.CoverStart + fifth, to = tunnel.CoverEnd - fifth;
                    float capFrom = hf.SampleWithoutTunnels(tv[from].Position.X, tv[from].Position.Z);
                    foreach (var _ in Settle(new Vector3(tv[from].Position.X, capFrom + m.BallRadius + 0.6f, tv[from].Position.Z), 0.6f)) yield return null;
                    rig2.SnapYawToward(new Vector3(Mathf.Cos(tv[from].Heading), 0f, Mathf.Sin(tv[from].Heading)));
                    Shot("cap_drive_over_the_tunnel", 40);
                    int cn = from, ct = 0, capTicks = 0, capGrounded = 0; float minOverCap = float.MaxValue; bool capReached = false;
                    while (ct++ < Engine.PhysicsTicksPerSecond * 20)
                    {
                        Vector3 p = _player.GlobalPosition;
                        float best = float.MaxValue;
                        for (int i = Mathf.Max(from, cn - 5); i < Mathf.Min(tv.Count, cn + 60); i++) { float d = new Vector2(tv[i].Position.X - p.X, tv[i].Position.Z - p.Z).LengthSquared(); if (d < best) { best = d; cn = i; } }
                        capTicks++;
                        if (_player.IsGrounded) capGrounded++;
                        minOverCap = Mathf.Min(minOverCap, p.Y - hf.SampleWithoutTunnels(p.X, p.Z));
                        if (cn >= to) { capReached = true; break; }
                        var tg = tv[Mathf.Min(tv.Count - 1, cn + 8)].Position;
                        _worldDrive = new Vector3(tg.X - p.X, 0f, tg.Z - p.Z);
                        yield return null;
                    }
                    ReleaseAll();
                    float capFrac = capGrounded / (float)Mathf.Max(1, capTicks);
                    GD.Print($"[SELFTEST] cap drive (seed {world.Seed}): {capTicks} ticks over the covered run ({tv[to].Distance - tv[from].Distance:0} m), grounded {capFrac:P0}, lowest {minOverCap:0.00} m over the cap's top, reached the far end {capReached}");
                    Check("driven along the surface over the covered run, the ball stays on the cap (docs/13 §2.8)", capReached && capFrac >= 0.9f && minOverCap > -0.5f, $"reached={capReached} grounded={capFrac:P0} lowest {minOverCap:0.00} m over the cap");
                }
            }
            else Check("stage has a covered tunnel to drive", true, $"none on this seed nor on seeds 1–60 of {ArchetypeRules.Label(world.Archetype)}");
        }
        ...
        {
            if (canyonSeed > 0 || tunnelSeed > 0 || skySeed > 0 || exitSeed > 0) { _debug.RestartSameSeed(); }   // back to the drive seed the regenerations rebuild
            foreach (var _ in Frames(3)) yield return null;
    ...
    /// <summary>Instruments 1 and 3, per seed: the wall probe on a rejected attempt 1, floor-3 room.</summary>
    private static void MeasureSeed(StageGenerator gen, StageGenerationRequest req, StageDefinition def,
                                    WallProbeTally wall, Floor3RoomTally room, TerrainArchetype archetype)
    {
    ...
    private IEnumerable RunShowcaseCase()
    {
        ...
        {
            t.World.SampleStage = System.Array.FindIndex(SampleStages.All, s => s.Name == "three exits") + 1;
            foreach (var _ in Frames(6)) yield return null;
    ...
    private IEnumerable RunStageLifecycleCase()
    {
        ...
        var sample = Rushcore.Generation.SampleStages.All.First(e => e.Name == "three exits");
        ...
        {
            Check("the lifecycle sample carries a terminal line to exit by", false, $"the \"three exits\" sample (Highlands seed {seed}) has no terminal line");
            t.World.GeneratedStage = false;
            t.World.Archetype = 0f;
