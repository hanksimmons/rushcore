...
The accepted movement baseline in `docs/03 §15` and `DECISIONS.md` (D-095: the user's `manual-small-3` preset, D-091 plus the slam and VFX overrides, promoted verbatim on 2026-09-06 on top of D-078; **its VFX half re-promoted 2026-09-22 by D-121**) is frozen input to generation. Do not retune it; a change only enters through a saved preset that the user names final, promoted verbatim and logged — `08`'s freeze guard loads `tuning/presets/manual-small-3.json` and asserts it equals the compiled defaults, so the preset and the defaults move together or not at all. Flow headroom (D-088) is part of that baseline (0.715); with headroom 0 the controller is the base-cap kit.
