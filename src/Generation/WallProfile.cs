...
/// on the facets. Every function takes the fillet radius as a parameter (D-113): the canyon's wall uses 30 m, a
/// tunnel's trench 6 m (<see cref="TunnelProfile"/>).
/// </summary>
public static class WallProfile
{
    ...
    /// <summary>The fillet's end: lateral extent and height, for a foot slope s0, a face slope faceTan and a fillet radius.</summary>
    private static void FilletEnd(float s0, float faceTan, float r, out float uEnd, out float hEnd, out float phi0)
    {
        phi0 = Mathf.Atan(s0);
        float theta = Mathf.Atan(faceTan);
        uEnd = r * (Mathf.Sin(theta) - Mathf.Sin(phi0));
        hEnd = r * (Mathf.Cos(phi0) - Mathf.Cos(theta));
    }

    /// <summary>Height above the origin at lateral distance u (0 for u ≤ 0).</summary>
    public static float Height(float u, float s0 = 0f, float? faceTan = null, float? radius = null)
    {
        float ft = faceTan ?? FaceTan, r = radius ?? FootRadius;
        if (u <= 0f) return 0f;
        FilletEnd(s0, ft, r, out float uEnd, out float hEnd, out float phi0);
        if (u < uEnd)
        {
            float phi = Mathf.Asin(Mathf.Clamp(u / r + Mathf.Sin(phi0), -1f, 1f));
            return r * (Mathf.Cos(phi0) - Mathf.Cos(phi));
        }
        return hEnd + (u - uEnd) * ft;
    }

    /// <summary>dHeight/du.</summary>
    public static float Slope(float u, float s0 = 0f, float? faceTan = null, float? radius = null)
    {
        float ft = faceTan ?? FaceTan, r = radius ?? FootRadius;
        if (u <= 0f) return s0;
        FilletEnd(s0, ft, r, out float uEnd, out _, out float phi0);
        if (u < uEnd)
        {
            float phi = Mathf.Asin(Mathf.Clamp(u / r + Mathf.Sin(phi0), -1f, 1f));
    ...
    public static float Curvature(float u, float s0 = 0f, float? faceTan = null, float? radius = null)
    {
        float r = radius ?? FootRadius;
        if (u <= 0f) return 0f;
        FilletEnd(s0, faceTan ?? FaceTan, r, out float uEnd, out _, out _);
        return u < uEnd ? 1f / r : 0f;
    }

    /// <summary>Lateral distance at which the profile reaches a height (the inverse of <see cref="Height"/>).</summary>
    public static float LateralAtHeight(float h, float s0 = 0f, float? faceTan = null, float? radius = null)
    {
        float ft = faceTan ?? FaceTan, r = radius ?? FootRadius;
        if (h <= 0f) return 0f;
        FilletEnd(s0, ft, r, out float uEnd, out float hEnd, out float phi0);
        if (h < hEnd)
        {
            ...
            float cosPhi = Mathf.Clamp(Mathf.Cos(phi0) - h / r, -1f, 1f);
            float phi = Mathf.Acos(cosPhi);
            return r * (Mathf.Sin(phi) - Mathf.Sin(phi0));
        }
    ...
    /// facets never touch it (a tunnel's 6 m fillet has a smaller sagitta and the same rest: a hair more air, no contact).</summary>
    ...
    /// the face by lateral step out to <paramref name="uMax"/>, for a fillet radius and face angle (the canyon's by default).</summary>
    public static float[] ShellStations(float uMax, float? radius = null, float? faceDegrees = null)
    {
        float r = radius ?? FootRadius, face = faceDegrees ?? WorldScale.WallFaceDegrees;
        var u = new List<float> { -WorldScale.WallShellFoot, -WorldScale.WallShellFoot * 0.5f, 0f };
        for (float deg = WorldScale.WallShellStepDegrees; deg <= face + 1e-3f; deg += WorldScale.WallShellStepDegrees)
            u.Add(r * Mathf.Sin(Mathf.DegToRad(deg)));
