...
RUSHCORE_ARCHETYPE=canyon <same command> --seed 3               # the stage case drives a Canyon Run stage (D-098); RUSHCORE_WALL_TRACE=1 prints the canyon wall ride per tick (D-109); RUSHCORE_WALL_AIM=<deg> rides at one aim only (default 20° then 90°); RUSHCORE_WALL_PROBE=1 prints the stamp/grid/analytic-normal probe along the wall (D-110); inside a RUSHCORE_DRIVE_TRACE window a vertical-speed jump over 30 m/s prints `[TRACE-JUMP]` with the sink, stamp and grid under the ball and every wall-shell triangle within 6 m (D-111)
